...
// Complete the in-flight message and mark the FW queue free.  The owning fd
// (unless it abandoned) receives the response, or, when the exchange failed,
// the errno its POLL will return.  Caller holds arc_msg_mutex.
static void arc_msg_deliver(struct tenstorrent_device *tt_dev, const struct arc_msg *resp, int error)
{
...
static void arc_msg_flush_inflight(struct tenstorrent_device *tt_dev, u32 queue_base, u32 num_entries)
{
	...
	arc_msg_deliver(tt_dev, &resp, ret);
}
...
int arc_msg_send_sync(struct tenstorrent_device *tt_dev, struct arc_msg *msg)
{
	...
	// The ring holds at most num_entries responses and nothing is legally
	// producing more, so a bounded drain empties any sane queue; the bound
	// keeps runaway pointers from spinning us forever.
	arc_msg_flush_inflight(tt_dev, queue_base, num_entries);
	for (i = 0; i < num_entries; i++)
		if (arc_msg_try_pop(tt_dev, &drain, queue_base, num_entries) != 0)
			break;
...
int arc_msg_pump(struct tenstorrent_device *tt_dev)
{
	...
	if (!list_empty(&tt_dev->arc_msg_queue)) {
		...
		if (ret != 0) {
			// Can't submit; complete with the errno for POLL to report.
			next->arc_msg.error = ret;
