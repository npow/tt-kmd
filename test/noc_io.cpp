...
// the read reached the requested endpoint.
...
void VerifyRoundtrip(int fd, uint16_t x, uint16_t y)
{
    ...
    for (const auto &[width, mask] : cases) {
        uint64_t addr = base + offset;
        uint64_t value = RNG() & mask;

        noc_write(fd, x, y, 0, width, addr, value);
        uint64_t got = noc_read(fd, x, y, 0, width, addr) & mask;
...
void VerifyValidation(int fd)
{
    ...
    { auto r = good(); r.width = 3;                  ExpectReadRejected(fd, r, "unsupported width"); }
    { auto r = good(); r.width = 0;                  ExpectReadRejected(fd, r, "zero width"); }
    { auto r = good(); r.width = 4; r.addr = 2;      ExpectReadRejected(fd, r, "misaligned 4-byte"); }
    { auto r = good(); r.width = 2; r.addr = 1;      ExpectReadRejected(fd, r, "misaligned 2-byte"); }
...
void VerifyWormholeAddressValidation(int fd)
{
    ...
    r.width = 4;
    r.addr = 1ULL << 36;
    ExpectReadRejected(fd, r, "address exceeds Wormhole NOC range");
}
