...
enum chardev_msg_state {
	...
	CHARDEV_MSG_COMPLETED,	// response in buf or errno in error; legal: POLL (consumes it, back to IDLE), ABANDON
};

struct chardev_msg {
	enum chardev_msg_state state;
	struct arc_msg buf;		// request on POST, overwritten by the response
	int error;			// 0 if buf holds a response; else the errno POLL returns
